...
class Modbus extends events.EventEmitter {
  constructor(mqtt, modbus, device) {
    ...
    //this.descriptor = modbusHerdsmanConverters.findByModbusModel(this.model); // by BoogieBug, using the code below instead
    ...
    this.descriptor = modbusHerdsmanConverters.findByModbusModel(this.model) || drivers.find( driver => driver.model == this.model ) || null;
    if ( ! this.descriptor ) {
  ...
  async pollRegistersByFC(functionCode = 4) {
    ...
    for (let key of keys) {
      ...
      // -- added by Boogiebug
      const address = parseInt(_.get(addressDescriptor, 'address'));
      const type    = _.get(addressDescriptor, 'type') || 'int16';
      const length  = parseInt(_.get(addressDescriptor, 'length')) || 1
      ...
      try {
        ...
        */
        ...
        if ( err.errno == 'ETIMEDOUT') {
          logger.error('MODBUS reading timeout');
          continue;
        }
        throw err;
      }
      ...
      */
      ...
      if ( type == 'float' || type == 'float32' ) {
        value = value.buffer.readFloatBE();
      }
      else {
        value = value.buffer.readInt16LE();
      }
      dataAvailable = true;
